...
export default function CourseCard(props) {
  const course = props.data // 取得商品數據
  ...
  const icon_click = (e, action) => {
    ...
    // API URL
    const api = `http://localhost:3005/api/user/course_like/${action}`
  ...
  return (
    <>
      <div className="col-12 col-sm-4 col-md-3" key={course.id}>
        <div onClick={CardClick} style={{ cursor: 'pointer' }}>
          <div id={styles['Product-component']} className="mx-2 my-3">
            <div className={styles['img-box']}>
              <img
                src={`http://localhost:3005/images/course/${course.img_1}`}
                alt="商品圖片"
              />
            </div>
            <p className={styles['content']}>{course.name}</p>
